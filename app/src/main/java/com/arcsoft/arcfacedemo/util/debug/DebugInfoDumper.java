...
public class DebugInfoDumper {
    ...
    /**
     * 特征提取失败的图像，命名为：
     * {width}x{height}_{timestamp}_{ERROR_TYPE}_{faceId}_{extractCode}_{rect(left-top-right-bottom)}_{orient}_{extractCost}.NV21
     */
    private static final String NAME_FORMAT_EXTRACT_ERROR_FILE = "%dx%d_%d_%d_%d_%d_(%d,%d,%d,%d)_%d_%d.NV21";
    ...
    public static String getExtractFailedFileName(int width, int height, int trackId, int extractCode, FaceInfo faceInfo, long extractCost) {
        Rect rect = faceInfo.getRect();
        return String.format(Locale.ENGLISH, NAME_FORMAT_EXTRACT_ERROR_FILE, width, height, System.currentTimeMillis(),
                ...
                faceInfo.getOrient(), extractCost);
    }
