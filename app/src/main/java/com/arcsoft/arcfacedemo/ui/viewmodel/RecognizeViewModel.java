...
public class RecognizeViewModel extends ViewModel implements RecognizeCallback {
    ...
    public  RegisterAndRecognizeActivity reg;
    public void setContext(RegisterAndRecognizeActivity registe){
       this.reg = registe;
    }
    ...
    private void registerFace(final byte[] nv21, FacePreviewInfo facePreviewInfo) {
        ...
        Observable.create((ObservableOnSubscribe<Boolean>) emitter -> {
            ...
            boolean success = FaceServer.getInstance().registerNv21(ArcFaceApplication.getApplication(), nv21.clone(), previewSize.width, previewSize.height,
                    facePreviewInfo.getFaceInfoRgb(), "registered_" + faceHelper.getTrackedFaceCount());
    ...
    public void init() {
        ...
        RecognizeConfiguration configuration = new RecognizeConfiguration.Builder()
                ...
                .imageQualityThreshold(ConfigUtil.getImageQualityThreshold(context))
        ...
        ftInitCode.postValue(ftEngine.init(context, DetectMode.ASF_DETECT_MODE_VIDEO, ConfigUtil.getFtOrient(context),
                ConfigUtil.getRecognizeScale(context), ConfigUtil.getRecognizeMaxDetectFaceNum(context), ftEngineMask));
        ...
        if (enableLiveness) {
            ...
            flInitCode.postValue(flEngine.init(context, DetectMode.ASF_DETECT_MODE_IMAGE, DetectFaceOrientPriority.ASF_OP_0_ONLY,
                    16, 10, livenessType == LivenessType.RGB ? FaceEngine.ASF_LIVENESS : FaceEngine.ASF_IR_LIVENESS));
        }
...
//        }
    ...
    public List<FacePreviewInfo> onPreviewFrame(byte[] nv21, boolean doRecognize) {
        if (faceHelper != null) {
            ...
            if (registerStatus == REGISTER_STATUS_READY && facePreviewInfoList.size() > 0) {
