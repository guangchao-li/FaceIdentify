...
public class FaceServer {
    ...
    public boolean registerNv21(Context context, byte[] nv21, int width, int height, FaceInfo faceInfo, String name) {
        ...
        int code;
        synchronized (faceEngine) {
            code = faceEngine.extractFaceFeature(nv21, width, height, FaceEngine.CP_PAF_NV21, faceInfo, faceFeature);
        }
...
//            throw new RuntimeException("crop image failed, code is " + cropCode);
    ...
    public boolean compareTowFace(FaceFeature faceFeature,FaceFeature face2Feature) {
        if (faceEngine == null ||  faceFeature == null || face2Feature==null) {
            return false;
        }

        FaceSimilar faceSimilar = new FaceSimilar();
        faceEngine.compareFaceFeature(faceFeature, face2Feature, faceSimilar);
